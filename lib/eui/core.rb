...
class Rect < Struct.new(:x, :y, :w, :h)
    ...
    def with_w(new_w) Rect.new(self.x, self.y, new_w, self.h) end
    def with_h(new_h) Rect.new(self.x, self.y, self.w, new_h) end
end
...
module EventHandlingMixin
	...
	@event_handlers.reverse_each do |handler|
		...
		rect = handler[:rect]
		yield handler if rect.nil? or rect.contains(event.x, event.y)
	    end
	end
    end
end
